...
async function installWrapper (pathToCLI, platform, arch) {
  ...
  try {
    source = [pathToCLI, `vesctl.${platform}-${arch}`].join(path.sep);
...
async function addCredentials (credentialsHostname, credentialsCertificate) {
  ...
  // eslint-disable
  const conf = `
server-urls: "${credentialsHostname}"
p12-bundle: ${process.env.HOME}/vesctl-certificate-bundle.p12`.trim();
  ...
  const creds = `${credentialsCertificate}`.trim();
    // eslint-enable

  let credsFile = `${process.env.HOME}/vesctl-certificate-bundle.p12`;
  
  // get containing folder
  const credfFolder = path.dirname(default);
...
async function run () {
  try {
    ...
    core.debug(`Getting url for vesctl version ${release.version}: ${platform} ${arch}`);
